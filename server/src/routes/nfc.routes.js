...
const scan_id_util_1 = require("../utils/scan-id.util");
const device_api_key_middleware_1 = require("../middleware/device-api-key.middleware");
const rate_limit_middleware_1 = require("../middleware/rate-limit.middleware");
const router = express_1.default.Router();
router.use(rate_limit_middleware_1.deviceBiometricLimiter);
router.use(device_api_key_middleware_1.verifyDeviceApiKey);
// Endpoint pour recevoir un scan NFC depuis un appareil externe
router.post('/scan', [
    (0, express_validator_1.body)('nfcId').notEmpty().withMessage('nfcId est requis'),
    (0, express_validator_1.body)('date').optional().isISO8601().withMessage('Format de date invalide'),
    (0, express_validator_1.body)('courseId').optional().isString().withMessage('courseId doit être une chaîne'),
    (0, express_validator_1.body)('autoStatus').optional().isIn(['PRESENT', 'LATE']).withMessage('autoStatus doit être PRESENT ou LATE'),
], async (req, res) => {
    try {
        ...
        let student = await prisma_1.default.student.findFirst({
            where: (0, scan_id_util_1.matchStudentScanId)(nfcId),
            include: {
                user: {
                    select: {
                        id: true,
        ...
        const staffMember = await prisma_1.default.staffMember.findFirst({
            where: (0, scan_id_util_1.matchStaffScanId)(nfcId),
            include: {
                user: {
        ...
        let teacher = await prisma_1.default.teacher.findFirst({
            where: (0, scan_id_util_1.matchTeacherScanId)(nfcId),
            include: {
                user: {
                    select: {
                        id: true,
...
router.get('/info/:nfcId', async (req, res) => {
    try {
...
//# sourceMappingURL=nfc.routes.js.map
...
const financial_breakdown_util_1 = require("../utils/financial-breakdown.util");
const router = express_1.default.Router();
function reportSchoolCtx(req) {
    const schoolId = req.schoolId;
    const isDefault = req.school?.isDefault ?? false;
    return {
        schoolId,
        isDefault,
        studentWhere: (0, school_context_util_1.studentScopeWhere)(schoolId, isDefault),
        classWhere: (0, school_context_util_1.classScopeWhere)(schoolId, isDefault),
        admissionWhere: (0, school_context_util_1.admissionScopeWhere)(schoolId, isDefault),
        accountingWhere: (0, school_context_util_1.accountingScopeWhere)(schoolId, isDefault),
    };
}
function norm20(score, maxScore) {
...
router.get('/reports/academic', async (req, res) => {
    try {
        ...
        for (const sa of studentAverages) {
            if (!sa.classId || sa.average20 == null)
                continue;
            const cs = classSuccess.get(sa.classId);
            if (!cs)
                continue;
            cs.evaluated += 1;
            if (sa.average20 >= PASS_THRESHOLD)
                cs.passed += 1;
        }
        let globalEvaluated = 0;
        let globalPassed = 0;
        for (const sa of studentAverages) {
            if (sa.average20 == null)
                continue;
            globalEvaluated += 1;
            if (sa.average20 >= PASS_THRESHOLD)
                globalPassed += 1;
        }
        const globalSuccessRate = globalEvaluated > 0 ? Math.round((globalPassed / globalEvaluated) * 1000) / 10 : null;
        const classComparison = [...classSuccess.entries()]
            .map(([cid, v]) => ({
            classId: cid,
            className: v.className,
            level: v.level,
            average20: v.pooledCoef > 0 ? Math.round((v.pooledSum / v.pooledCoef) * 100) / 100 : null,
        ...
        for (const g of grades) {
            ...
            if (!monthMap.has(key))
                monthMap.set(key, { sum: 0, coef: 0, count: 0 });
        ...
        const bullets = [];
        bullets.push(`${grades.length} note(s) prise(s) en compte sur la période « ${periodLabel} »` +
            (academicYear ? ` (${academicYear})` : '') +
            '.');
        ...
        if (globalSuccessRate != null) {
            bullets.push(`Taux de réussite (moyenne individuelle ≥ ${PASS_THRESHOLD}/20) : ${globalSuccessRate} % (${globalPassed} / ${globalEvaluated} élèves évalués).`);
        }
        if (classComparison.length > 0) {
            const top = classComparison[0];
            bullets.push(`Classe la plus haute moyenne pondérée : ${top.className}${top.average20 != null ? ` (${top.average20}/20)` : ''}.`);
        }
        res.json({
            filters: {
...
function academicYearToSchoolRange(academicYear) {
    const parts = academicYear.split('-').map((x) => parseInt(x.trim(), 10));
    if (parts.length < 2 || parts.some((n) => !Number.isFinite(n)))
        return null;
    const yearStart = parts[0];
...
router.get('/reports/administrative', async (req, res) => {
    try {
        ...
        const now = new Date();
        let rangeStart;
        let rangeEnd = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59, 999);
        if (fromQ && toQ) {
        ...
        const [studentsTotalAll, studentsActiveAll, studentsTotalScoped, studentsActiveScoped, studentsWithoutClass, enrollmentGroups, teachersTotalAll, educatorsTotal, staffTotal, staffByCategory, classesTotalAll, classesScoped, coursesInYearTeachers, absenceStatusGroups, teacherAttGroups, staffAttGroups, admissionStatusGroups, admissionByYear, transfersInRange, newEnrollments, archivedStudents,] = await Promise.all([
            prisma_1.default.student.count({ where: studentWhere }),
            prisma_1.default.student.count({ where: { ...studentWhere, isActive: true, enrollmentStatus: 'ACTIVE' } }),
            prisma_1.default.student.count({ where: studentClassYearWhere }),
            ...
            prisma_1.default.student.count({ where: { ...studentWhere, classId: null } }),
            ...
            prisma_1.default.staffMember.count({ where: { schoolId: req.schoolId } }),
            prisma_1.default.staffMember.groupBy({
                by: ['staffCategory'],
                where: { schoolId: req.schoolId },
                _count: true,
            }),
            prisma_1.default.class.count({ where: classWhere }),
            academicYear ? prisma_1.default.class.count({ where: classYearFilter }) : prisma_1.default.class.count({ where: classWhere }),
            academicYear
                ...
                : Promise.resolve([]),
            prisma_1.default.absence.groupBy({
                by: ['status'],
                where: absenceWhere,
                _count: true,
            }),
            prisma_1.default.teacherAttendance.groupBy({
                by: ['status'],
                where: {
            ...
            prisma_1.default.staffAttendance.groupBy({
                by: ['status'],
                where: {
                    attendanceDate: { gte: dateKeyFrom, lte: dateKeyTo },
                },
            ...
            prisma_1.default.studentTransfer.findMany({
                where: {
        ...
        const stAbs = statusCounts(absenceStatusGroups);
        const teAtt = statusCounts(teacherAttGroups);
        const stfAtt = statusCounts(staffAttGroups);
        const sumStatuses = (m, keys) => keys.reduce((s, k) => s + (m[k] ?? 0), 0);
        const studentPresDenom = sumStatuses(stAbs, ['PRESENT', 'LATE', 'ABSENT']);
        const studentPresNum = sumStatuses(stAbs, ['PRESENT', 'LATE']);
        const studentPresenceRate = studentPresDenom > 0 ? Math.round((studentPresNum / studentPresDenom) * 1000) / 10 : null;
        const teacherDenom = sumStatuses(teAtt, ['PRESENT', 'LATE', 'ABSENT', 'EXCUSED']);
        const teacherNum = sumStatuses(teAtt, ['PRESENT', 'LATE']);
        const teacherPresenceRate = teacherDenom > 0 ? Math.round((teacherNum / teacherDenom) * 1000) / 10 : null;
        const staffDenom = sumStatuses(stfAtt, ['PRESENT', 'LATE', 'ABSENT', 'EXCUSED']);
        const staffNum = sumStatuses(stfAtt, ['PRESENT', 'LATE']);
        const staffPresenceRate = staffDenom > 0 ? Math.round((staffNum / staffDenom) * 1000) / 10 : null;
        ...
        const studentsPerTeacher = teachersForRatio > 0 ? Math.round((studentsForRatio / teachersForRatio) * 100) / 100 : null;
        const studentsPerClass = classesForRatio > 0 ? Math.round((studentsForRatio / classesForRatio) * 100) / 100 : null;
        const teachersPerClass = classesForRatio > 0 ? Math.round((teachersForRatio / classesForRatio) * 100) / 100 : null;
        ...
        const admissionMap = Object.fromEntries(admissionStatusGroups.map((r) => [
            r.status,
            gbCount(r),
        ]));
        res.json({
...
router.get('/reports/financial', async (req, res) => {
    try {
        ...
        const tuitionYearFilter = academicYear
            ...
            : { student: studentWhere };
        const [paymentByStatus, unpaidTotals, unpaidByFeeType, overdueUnpaid, completedInPeriod, expenseByCategory, pettyByType, budgetYearRows,] = await Promise.all([
            ...
            prisma_1.default.tuitionFee.groupBy({
                by: ['feeType'],
                where: { isPaid: false, ...tuitionYearFilter },
                _sum: { amount: true },
                _count: true,
            ...
            prisma_1.default.schoolExpense.groupBy({
                by: ['category'],
                where: {
                    ...accountingWhere,
            ...
            prisma_1.default.pettyCashMovement.groupBy({
                by: ['type'],
                where: {
                    ...accountingWhere,
            ...
            prisma_1.default.budgetLine.groupBy({
                by: ['academicYear'],
        ...
        const budgetYearResolved = academicYear ||
        ...
        const expenseActualGroups = budgetRange != null
            ? await prisma_1.default.schoolExpense.groupBy({
                by: ['category'],
                where: {
                    ...accountingWhere,
        ...
        const budgetSumByCategory = new Map();
        for (const bl of budgetLines) {
            budgetSumByCategory.set(bl.category, (budgetSumByCategory.get(bl.category) ?? 0) + bl.budgetedAmount);
        }
        const budgetVsActualLines = budgetLines.map((line) => {
            ...
            const variancePercent = line.budgetedAmount > 0 ? Math.round((variance / line.budgetedAmount) * 1000) / 10 : null;
            return {
                id: line.id,
                label: line.label,
        ...
        const budgetTotalsAgg = budgetVsActualLines.reduce((acc, row) => ({
        ...
        const revenueByFeeType = new Map();
        ...
        const [rev3m, exp90] = await Promise.all([
            ...
            prisma_1.default.schoolExpense.aggregate({
                where: {
                    ...accountingWhere,
        ...
        const avgMonthlyCompletedRevenue = Math.round(((rev3m._sum.amount ?? 0) / 3) * 100) / 100;
        const avgMonthlyExpenses = Math.round(((exp90._sum.amount ?? 0) / 3) * 100) / 100;
        ...
        const projectedRevenueTrend = monthsRemaining > 0 && avgMonthlyCompletedRevenue > 0
            ? Math.round(avgMonthlyCompletedRevenue * monthsRemaining * 100) / 100
            : null;
        const projectedExpensesTrend = monthsRemaining > 0 && avgMonthlyExpenses > 0
        ...
        const pettyIn = pettyByType.find((r) => r.type === 'IN')?._sum.amount ?? 0;
        const pettyOut = pettyByType.find((r) => r.type === 'OUT')?._sum.amount ?? 0;
        ...
        res.json({
            ...
            budgetVsActual: {
                ...
                expenseScopeNote: budgetRange != null
 ...
 */
router.get('/reports/student-stats', async (req, res) => {
    try {
        ...
        const genderByLevel = [...genderByLevelMap.values()].sort((a, b) => a.level.localeCompare(b.level, 'fr'));
        const courseWhere = {
            class: {
                ...classWhere,
                ...(academicYear ? { academicYear } : {}),
        ...
        const isStateAssigned = (raw) => raw === 'STATE_ASSIGNED';
        ...
        const notAssignedStudentIds = new Set();
        const byClass = new Map();
        const byLevel = new Map();
        const emptyBucket = (id, name, level, academicYear) => ({
            id,
            name,
            level,
            academicYear,
        ...
        for (const g of grades) {
            ...
            const className = meta?.className ?? cls?.name ?? classById.get(classId ?? '')?.name ?? 'Classe';
            const level = meta?.level ?? cls?.level ?? classById.get(classId ?? '')?.level ?? 'Sans niveau';
            const year = (classId && classById.get(classId)?.academicYear) ||
                (cls && 'academicYear' in cls ? cls.academicYear : null);
        ...
        const finishRecap = (acc) => {
            ...
            return {
                ...
                average20: acc.avgCount > 0
                    ? Math.round((acc.avgSum / acc.avgCount) * 100) / 100
                    : null,
            };
        };
        const ensureRecap = (map, id, name, level) => {
            let row = map.get(id);
            if (!row) {
                row = {
                    id,
        ...
        const recapByClassMap = new Map();
        const recapByLevelMap = new Map();
        ...
        for (const s of students) {
            ...
            if (assigned)
                levelRow.effectifAssigned += 1;
            else
                levelRow.effectifNotAssigned += 1;
            if (s.classId) {
                const classRow = ensureRecap(recapByClassMap, s.classId, s.class?.name ?? 'Classe', level);
                classRow.effectifTotal += 1;
                if (assigned)
                    classRow.effectifAssigned += 1;
                else
                    classRow.effectifNotAssigned += 1;
            }
        }
        for (const sa of studentAverages) {
            const band = sa.average20 >= 10 ? 'above' : sa.average20 >= 8.5 ? 'mid' : 'below';
            const applyBand = (row) => {
                row.graded += 1;
        ...
        for (const s of students) {
            if (isStateAssigned(s.stateAssignment))
                headcountAssigned += 1;
            else
                headcountNotAssigned += 1;
        }
        const headcountByClass = classes.map((c) => {
            ...
            return {
                ...
                capacity: c.capacity,
                fillRate: c.capacity > 0 ? Math.round((inClass.length / c.capacity) * 1000) / 10 : null,
            };
        });
        const roundAvg = (sum, coef) => coef > 0 ? Math.round((sum / coef) * 100) / 100 : null;
        res.json({
 ...
 */
router.get('/reports/mena', async (req, res) => {
    try {
        ...
        if (brandingDelegate) {
            ...
            brandingRow = (await brandingDelegate.findUnique({ where: { id: brandingId } }));
        }
        ...
        const classIds = classes.map((c) => c.id);
        const studentYearFilter = academicYear.length > 0
            ? classIds.length > 0
                ? { OR: [{ classId: { in: classIds } }, { classId: null }] }
                : { classId: null }
            : {};
        ...
        const brandingPublic = brandingRow
            ? (0, branding_assets_util_1.toPublicBrandingShape)(brandingRow)
            : null;
        const fiche = {
            schoolId,
            name: brandingPublic?.schoolDisplayName?.trim() ||
                school?.name ||
                null,
            ...
            principal: brandingPublic?.schoolPrincipal || school?.principalName || null,
        ...
        const completeness = {
            ...
            fneCoveragePercent: scopedStudents.length > 0
        ...
        res.json({
            ...
            effectifsByClass,
            effectifsByLevel,
            stateAssignedStudents: stateAssignedList.sort((a, b) => `${a.lastName} ${a.firstName}`.localeCompare(`${b.lastName} ${b.firstName}`, 'fr')),
            completeness,
        });
    }
    catch (e) {
 ...
 */
router.get('/reports/financial/breakdown', async (req, res) => {
    try {
        const { studentWhere, accountingWhere } = reportSchoolCtx(req);
        const academicYear = typeof req.query.academicYear === 'string' ? req.query.academicYear.trim() : '';
        const fromQ = typeof req.query.from === 'string' ? req.query.from.trim() : '';
        ...
        for (const g of Object.keys(financial_breakdown_util_1.GENDER_LABELS)) {
            byGender.set(g, (0, financial_breakdown_util_1.emptyMoneyBucket)(g, financial_breakdown_util_1.GENDER_LABELS[g]));
        }
        ...
        let unpaidCount = 0;
        let overdueCount = 0;
        const touch = (dim, kind, amount, overdue) => {
            const classKey = dim.classId ?? 'none';
            const classBucket = byClass.get(classKey) ?? (0, financial_breakdown_util_1.emptyMoneyBucket)(classKey, dim.className, dim.level);
            const levelBucket = byLevel.get(dim.level) ?? (0, financial_breakdown_util_1.emptyMoneyBucket)(dim.level, dim.level, dim.level);
            const genderBucket = byGender.get(dim.gender) ?? (0, financial_breakdown_util_1.emptyMoneyBucket)(dim.gender, financial_breakdown_util_1.GENDER_LABELS[dim.gender]);
            if (kind === 'paid') {
        ...
        for (const p of payments) {
            const dim = (0, financial_breakdown_util_1.studentDimFrom)(p.student);
            paidTotal += p.amount;
        ...
        for (const fee of unpaidFees) {
            ...
            if (remaining <= 0)
                continue;
            const dim = (0, financial_breakdown_util_1.studentDimFrom)(fee.student);
        ...
        const paidByClassStudents = new Map();
        const unpaidByClassStudents = new Map();
        const paidByLevelStudents = new Map();
        const unpaidByLevelStudents = new Map();
        const paidByGenderStudents = new Map();
        const unpaidByGenderStudents = new Map();
        for (const p of payments) {
            const dim = (0, financial_breakdown_util_1.studentDimFrom)(p.student);
            const ck = dim.classId ?? 'none';
            if (!paidByClassStudents.has(ck))
                paidByClassStudents.set(ck, new Set());
            paidByClassStudents.get(ck).add(p.studentId);
            if (!paidByLevelStudents.has(dim.level))
                paidByLevelStudents.set(dim.level, new Set());
            paidByLevelStudents.get(dim.level).add(p.studentId);
            if (!paidByGenderStudents.has(dim.gender))
                paidByGenderStudents.set(dim.gender, new Set());
            paidByGenderStudents.get(dim.gender).add(p.studentId);
        }
        for (const fee of unpaidFees) {
            const paidOnFee = fee.payments.reduce((s, x) => s + x.amount, 0);
            if (fee.amount - paidOnFee <= 0)
                continue;
            const dim = (0, financial_breakdown_util_1.studentDimFrom)(fee.student);
            const ck = dim.classId ?? 'none';
            if (!unpaidByClassStudents.has(ck))
                unpaidByClassStudents.set(ck, new Set());
            unpaidByClassStudents.get(ck).add(fee.studentId);
            if (!unpaidByLevelStudents.has(dim.level))
                unpaidByLevelStudents.set(dim.level, new Set());
            unpaidByLevelStudents.get(dim.level).add(fee.studentId);
            if (!unpaidByGenderStudents.has(dim.gender))
                unpaidByGenderStudents.set(dim.gender, new Set());
        ...
        const expensesRows = expenseByCategory.map((r) => ({
            category: r.category,
            count: gbCount(r),
            totalAmount: (0, financial_breakdown_util_1.roundMoney)(r._sum.amount ?? 0),
        }));
        res.json({
            filters: {
                academicYear: academicYear || null,
            ...
            overview: {
                paidAmount: (0, financial_breakdown_util_1.roundMoney)(paidTotal),
                paidCount: payments.length,
                studentsWithPayments: paidStudentIds.size,
                unpaidAmount: (0, financial_breakdown_util_1.roundMoney)(unpaidTotal),
                unpaidCount,
                studentsWithUnpaid: unpaidStudentIds.size,
                overdueAmount: (0, financial_breakdown_util_1.roundMoney)(overdueTotal),
                overdueCount,
                expensesAmount: (0, financial_breakdown_util_1.roundMoney)(expenseTotal._sum.amount ?? 0),
                expensesCount: expenseTotal._count,
                netEncaissementsMoinsDepenses: (0, financial_breakdown_util_1.roundMoney)(paidTotal - (expenseTotal._sum.amount ?? 0)),
            },
            paymentsAndUnpaid: {
                byClass: (0, financial_breakdown_util_1.finalizeMoneyBuckets)(byClass),
                byLevel: (0, financial_breakdown_util_1.finalizeMoneyBuckets)(byLevel),
                byGender: (0, financial_breakdown_util_1.finalizeMoneyBuckets)(byGender).filter((b) => b.paidCount > 0 || b.unpaidCount > 0 || b.key !== 'UNKNOWN'),
            },
            expensesByCategory: expensesRows,
            genderLabels: financial_breakdown_util_1.GENDER_LABELS,
        });
    }
    catch (e) {
...
//# sourceMappingURL=admin-reports.routes.js.map
...
exports.logProductionEnvDiagnostics = logProductionEnvDiagnostics;
exports.prismaConnectionErrorMessage = prismaConnectionErrorMessage;
exports.databaseUrlMisconfigurationHint = databaseUrlMisconfigurationHint;
exports.logDatabaseUrlDiagnostics = logDatabaseUrlDiagnostics;
/** Journalise les variables manquantes en production (sans exposer de secrets). */
function logProductionEnvDiagnostics() {
    if (process.env.NODE_ENV !== 'production')
        return;
    const missing = [];
    const jwt = (process.env.JWT_SECRET ?? '').trim();
    if (!jwt || jwt.length < 32)
        missing.push('JWT_SECRET (≥ 32 caractères)');
    ...
    if (!nfc || nfc.length < 32) {
        console.warn('[Config] NFC_API_KEY absent ou faible — les routes NFC / reconnaissance faciale seront indisponibles jusqu’à configuration.');
    }
    if (!process.env.SENSITIVE_FIELD_ENCRYPTION_KEY?.trim()) {
        console.warn('[Config] SENSITIVE_FIELD_ENCRYPTION_KEY absent — champs sensibles élève non chiffrés.');
    }
    ...
    if (missing.length > 0) {
        console.error(`[Config] Variables obligatoires manquantes en production : ${missing.join(', ')}`);
    }
}
/** Message d’erreur utilisateur pour échec Prisma (connexion, timeout). */
function prismaConnectionErrorMessage(error) {
...
function databaseUrlMisconfigurationHint(url) {
    ...
    const authority = pathStart >= 0 ? withoutScheme.slice(0, pathStart) : withoutScheme.split('?')[0] ?? '';
...
function logDatabaseUrlDiagnostics() {
    const db = (process.env.DATABASE_URL ?? '').trim();
    if (!db)
        return;
...
//# sourceMappingURL=production-env-diagnostics.util.js.map
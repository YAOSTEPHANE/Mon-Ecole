...
const KINDS = ['EBOOK', 'PDF', 'PEDAGOGICAL'];
const ROLES = ['STUDENT', 'TEACHER', 'PARENT', 'EDUCATOR', 'STAFF'];
...
router.post('/library/digital-resources', [
    (0, express_validator_1.body)('title').trim().notEmpty(),
    (0, express_validator_1.body)('kind').isIn(KINDS),
    (0, express_validator_1.body)('fileUrl').trim().notEmpty(),
    (0, express_validator_1.body)('onlineAccessEnabled').optional().isBoolean(),
    (0, express_validator_1.body)('tempDownloadEnabled').optional().isBoolean(),
    (0, express_validator_1.body)('downloadTtlHours').optional().isInt({ min: 1, max: 168 }),
    (0, express_validator_1.body)('allowedRoles').optional().isArray(),
], async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty())
            return res.status(400).json({ errors: errors.array() });
        const { title, author, description, kind, fileUrl, fileName, mimeType, fileSizeBytes, coverImageUrl, subject, level, onlineAccessEnabled, tempDownloadEnabled, downloadTtlHours, allowedRoles, isActive, publishedAt, } = req.body;
        const roles = Array.isArray(allowedRoles)
            ? allowedRoles.map((r) => String(r).toUpperCase()).filter((r) => ROLES.includes(r))
            : [];
        const created = await prisma_1.default.digitalLibraryResource.create({
            data: {
...
router.put('/library/digital-resources/:id', async (req, res) => {
    try {
        const existing = await prisma_1.default.digitalLibraryResource.findUnique({ where: { id: req.params.id } });
        if (!existing)
            return res.status(404).json({ error: 'Ressource introuvable' });
        const b = req.body ?? {};
        const roles = b.allowedRoles !== undefined && Array.isArray(b.allowedRoles)
            ? b.allowedRoles.map((r) => String(r).toUpperCase()).filter((r) => ROLES.includes(r))
            : undefined;
        const updated = await prisma_1.default.digitalLibraryResource.update({
...
router.delete('/library/digital-resources/:id', async (req, res) => {
    try {
        ...
        if (!existing)
            return res.status(404).json({ error: 'Ressource introuvable' });
        await prisma_1.default.digitalLibraryResource.update({
            where: { id: req.params.id },
            data: { isActive: false },
        });
...
//# sourceMappingURL=digital-library-management.routes.js.map
...
export type SchoolReportCardBranding = {
  schoolName: string;
  schoolPhone: string;
...
async function resolveLogoDataUrlForPdf(branding: SchoolReportCardBranding): Promise<string | null> {
...
export const SCHOOL_REPORT_CARD_DEFAULT_BRANDING: SchoolReportCardBranding = {
...
/** Modèle de bulletin scolaire — 1er / 2ème trimestre (cf. bulletin papier). */
...
function drawOfficialHeader(
  doc: jsPDF,
  pageWidth: number,
  branding: SchoolReportCardBranding,
  academicYear: string,
  periodKey: string,
  logoDataUrl: string | null,
...
export async function generateSchoolReportCardPdf(
  studentData: ReportCardStudentPayload,
  options: {
    periodLabel: string;
    periodKey: string;
    academicYear: string;
    branding?: Partial<SchoolReportCardBranding>;
  },
): Promise<void> {
  const branding: SchoolReportCardBranding = {

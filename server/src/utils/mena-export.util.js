...
exports.buildMenaStudentExportPackage = buildMenaStudentExportPackage;
exports.checksumMenaPackage = checksumMenaPackage;
exports.menaPackageToStudentsCsv = menaPackageToStudentsCsv;
exports.pushMenaPackageToWebhook = pushMenaPackageToWebhook;
exports.getConfiguredMenaWebhookUrl = getConfiguredMenaWebhookUrl;
exports.maskConfiguredMenaWebhook = maskConfiguredMenaWebhook;
...
function isStateAssigned(raw) {
    return raw === 'STATE_ASSIGNED';
}
async function buildMenaStudentExportPackage(schoolId, isDefaultSchool, academicYear) {
    ...
    if (brandingDelegate) {
        ...
        if (row) {
            branding = (0, branding_assets_util_1.toPublicBrandingShape)(row);
        }
    }
    const classWhere = {
        ...(0, school_context_util_1.classScopeWhere)(schoolId, isDefaultSchool),
    ...
    return {
        meta: {
            ...
            purpose: 'Dossier complet élèves établissement pour remontée MENA / DESPS (identité, FNE, parents, contacts, documents, historique, effectifs, affectés État). Biométrie / NFC exclus.',
            academicYear: year,
            schoolId,
        },
        etablissement: {
        ...
        effectifs: {
            ...
            parClasse: [...byClass.values()].sort((a, b) => a.niveau.localeCompare(b.niveau, 'fr') || a.classe.localeCompare(b.classe, 'fr')),
        },
    };
}
function checksumMenaPackage(pkg) {
    return (0, crypto_1.createHash)('sha256').update(JSON.stringify(pkg)).digest('hex').slice(0, 32);
}
function menaPackageToStudentsCsv(pkg) {
    ...
    for (const e of pkg.eleves) {
        ...
        const u1 = e.contactUrgence1;
        const u2 = e.contactUrgence2;
...
async function pushMenaPackageToWebhook(pkg, webhookUrl) {
...
function getConfiguredMenaWebhookUrl() {
...
function maskConfiguredMenaWebhook() {
    const url = getConfiguredMenaWebhookUrl();
    return url ? maskUrl(url) : null;
}
//# sourceMappingURL=mena-export.util.js.map